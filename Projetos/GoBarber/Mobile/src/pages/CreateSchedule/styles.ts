...
interface IHourAvailableProps {
  available: boolean;
  selected: boolean;
}

interface IHourTextAvailableProps {
  selected: boolean;
}
...
export const Hour = styled(RectButton)<IHourAvailableProps>`
  padding: 12px;
  background: ${props => (props.selected ? '#ff9000' : '#3e3b47')};
  border-radius: 10px;
  margin-right: 8px;
...
export const HourText = styled.Text<IHourTextAvailableProps>`
  color: #f4ede8;
...
export const CreateScheduleButton = styled(RectButton)`
  height: 50px;
  background: #ff9000;
  border-radius: 10px;
  align-items: center;
...
export const CreateScheduleButtonText = styled.Text`
  font-family: 'RobotoSlab-Medium';
  color: #232129;
  font-size: 18px;
`;
